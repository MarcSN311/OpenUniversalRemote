...
Keypad_MC17 customKeypad( makeKeymap(hexaKeys), rowPins, colPins, ROWS, COLS, 0x20); 
...
byte virtualKeyMapHannspree[10] = {0x80, 0x81, 0x82, 0x84, 0x85, 0x86, 0x88, 0x89, 0x8A, 0x8C};
...
static void virtualKeypad_event_cb(lv_event_t* e) {
  ...
  Serial.println(virtualKeyMapHannspree[(int)target->user_data]);
  ...
  IrSender.sendNEC(0x33, virtualKeyMapHannspree[(int)target->user_data], 4);
}
...
void setup() {  
  ...
  digitalWrite(IR_LED, LOW); // HIGH off - LOW on
  ...
  touch_init(); // Initialize touchscreen and set sensitivity threshold
  ...
  // Setup IR
  IrSender.begin(IR_LED, DISABLE_LED_FEEDBACK, USE_DEFAULT_FEEDBACK_LED_PIN);
...
void loop() { 
  ...
  for(int i=0; i<LIST_MAX; i++){ // Handle multiple keys (Not really necessary in this case)
    if(customKeypad.key[i].kstate == PRESSED || customKeypad.key[i].kstate == HOLD){
      ...
      if(currentDevice == 1) IrSender.sendNEC(0x33, 0x83, 4);
      else if(currentDevice == 2) IrSender.sendNEC(0x33, 0x83, 4);
    }
  }
}